...
module Jamf
  ...
  #
  # IMPORTANT: Since SelfServable also includes #{Jamf::Uploadable}, for uploading icons,
  ...
  module SelfServable
    ...
    def self_service_notification_type=(type)
      ...
      raise Jamf::InvalidDataError, "type must be one of: :#{NOTIFICATION_TYPES.keys.join ', :'}" unless NOTIFICATION_TYPES.key? type
    ...
    #
    # Since Jamf::Icon objects are read-only,
...
<#{self.class::RSRC_OBJECT_KEY}>
  <self_service>
    ...
    def parse_self_service
      ...
      @icon = Jamf::Icon.new(ss_data[:self_service_icon]) if ss_data[:self_service_icon]
    ...
    def add_self_service_macos_xml(ssvc)
      ...
      return if self.class == Jamf::MacApplication
    ...
    def validate_user_removable(new_val)
      raise Jamf::UnsupportedError, 'User removal settings not applicable to this class' unless self_service_payload == :profile

      raise Jamf::UnsupportedError, 'Removal :with_auth not applicable to this class' if new_val == :with_auth && !self_service_targets.include?(:ios)

      raise Jamf::InvalidDataError, "Value must be one of: :#{PROFILE_REMOVAL_BY_USER.keys.join(', :')}" unless PROFILE_REMOVAL_BY_USER.key?(new_val)
    end

    # Raise an error if an icon id is not valid
    def validate_icon(id)
      return nil unless Jamf::DB_CNX.connected?
      raise Jamf::NoSuchItemError, "No icon with id #{id}" unless Jamf::Icon.all_ids.include? id
    end

    # Raise an error if notifications aren't supported
    def validate_notifications_supported
      raise Jamf::UnsupportedError, "#{self.class} doesn't support Self Service notifications" unless @self_service_data_config[:notifications_supported]
    end

    # Raise an error if notification reminders aren't supported
    def validate_notification_reminders_supported
      raise Jamf::UnsupportedError, "#{self.class} doesn't support Self Service notifications" unless @self_service_data_config[:notification_reminders]
    end
